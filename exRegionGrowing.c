...
printf("\n ..... (ARQUIVO GRAVADO)-->%s.", arq);
        ...
        if (!f) {
            ...
            return -1;  // algum código de erro - Arquivo nao existe.
        }
        ...
        if (!strcmp(tpPGM, "P3")) {/// a imagem é colorida???
                                      /// Se sim, largura está multiplicada por 3
            fprintf(f,"%d %d\n", pPgm->w/3, pPgm->h); // divide a largura por 3

        }else {
            fprintf(f,"%d %d\n", pPgm->w, pPgm->h); // imagem binária ou cinza.
        }
        fprintf(f,"255\n"); // valor de brilho máximo na imagem (use sempre 255
...
int ReadPGMP2(const char* arq, pgm* pPgm) {
    ...
    //              dados relevantes que estão gravados no arquivo PGM arq.
      ...
      if ((strcmp(readChars, "P2"))) { //tipo PGM não é válido? strcmp(s1,s2) compara os caracteres das
                                       //   strings s1 e s2 e retorna o valor zero se s1 é exatamente igual a s2
       ...
       while (readChars[0] == '#') {   // se no cabeçalho do arquivo existe uma linha de comentário,
          ...
          fscanf (fd, "%s", readChars); // Pular essa linha (ela é irrelevante)
        }
...
int main(){
    ...
    const char* NimgOR   = "D:\\ProgamasC\\UmuNirGeoEye.pgm"; /// imagem original cinza
    const char* NimgGROW = "D:\\ProgamasC\\UmuGrowGeoEye.pbm";/// imagem binária da silhueta do lago
    const char* NimgBORD = "D:\\ProgamasC\\UmuBordGeoEye.pbm";///imagem binária da borda do lago
...
///
    ...
    InicializaPilha(pilha); /// indicando que a pilha está vazia
...
//    printf("\n desempilhado ... (%d, %d) \n", y.L, y.C);
    ...
    int sim;  /// medida de similaridade de um pixel em relação ao
    ...
    while (pilha->topo) {  /// enquanto existir coordenada na pilha
        ...
        for (k = -1; k<=1; k++) {    /// elemento estruturante EE 3x3
            ...
            for (j = -1; j<=1; j++){
                ...
                ///       pixel já foi processado (1) ou não (0)
                sim = (abs(VS - Vpix) <= critsim);  /// pergunta (???): o valor do pixel Vpix
                 ///       de coordenada (y.L, y.C) armazenado na imagem original imgOR é
                 ///       similar (sim == 1) ou não é similar (sim==0) ao ponto semente??

        ///  se esse pixel de coordenada (y.L, y.C) é similar (sim == 1)
        ///  ao ponto semente e, ainda, se ele NÃO foi processado (Growpix == 0
        ///  que é o mesmo que !Growpix) temos que realizar duas ações:
        ///     (1)  marcar esse pixel na imagem resultado com o valor 1 e;
        ///     (2)  empilhar essa coordenada y .
        ///  A operação (1) diz que esse pixel pertence a região de interesse, e
...
///printf(" (%d, %d, %d) ", y.L, y.C, GetPixel(imgGROW, y.L, y.C));
    ...
    /// gerar borda colorida - a estrutura de uma imagem colorida tem 3 vezes mais pixels.
     ...
     const char* NColorBORDUmu  = "D:\\ProgamasC\\UmuColorBordGeoEye.ppm";
     ...
     imgCOLOR->max = 255; /// campo inútil (maior valor de brilho na imagem)

    /// observe que, por enquanto, temos apenas o descritor da imagem imgCOLOR.
    /// Precisamos alocar o reticulado de suporte da imagem (MATRIZ). Uma maneira é
    ...
    MemImgZero(imgCOLOR, imgCOLOR);  /// com isso, a imagem imgCOLOR tem estrutura de
                    /// uma imagem colorida e todos os pixels dela são pretos.


    /// vamos percorrer os pixels da imagem que contém somente a borda do objeto desejado
...
//
//             imggrow = malloc (sizeof(pgm));
//             MemImgZero(nirUmuPgm, imggrow);
//printf("\n zerou  %s \n", arqgrowUmu);
//             int critsim = 10;
//             int LS = 377, CS = 507;
//             int VS = GetPixel(nirUmuPgm, LS, CS);
//printf("\n pix sem  %d \n", VS);
//             TpPILHA *PilhaGrow;
//             InicializaPilha(PilhaGrow);
//             TpDADO x, y;
//             x.L = LS;
//             x.C = CS ;
//             PutPixel(imggrow, x.L, x.C, 1);
//             push(x, PilhaGrow); /// empilhar x = 20
//printf("\n (%d, %d)  %d \n", PilhaGrow->topo->dado.L, PilhaGrow->topo->dado.C);
//             while (PilhaGrow->topo) {
//                pop(&x, PilhaGrow);
//                for (int k = -1; k<=1; k++) {    /// elemento estruturante EE 3x3
//                    for (int j = -1; j<=1; j++){
//                            y.L = x.L + k;
//                            y.C = x.C + j ;
//                            Tpixel pix = GetPixel(nirUmuPgm, y.L, y.C);
//
//                            if (!pix && (abs(VS - pix)<= critsim)) {
//                                    PutPixel(imggrow, y.L, y.C, 1);
//                                    push(y, PilhaGrow);
//                            }
//                    }
//                }
...
/////    if (imggrow->pData[k]) printf(' %d ', imggrow->pData[k]);
/////}
//        WritePGM(arqgrowUmu, "P1", imggrow);
